...
function isTraversingArray(node) {
    ...
    return (
        ...
        (declarator = init.declarations[0]) &&
        ...
        test.left.name === declarator.id.name &&
        ...
        (
            (
                ...
                update.argument.name === declarator.id.name
            ) || (
                update.type === "AssignmentExpression" &&
                update.operator === "+=" &&
                update.left.type === "Identifier" &&
                update.left.name === declarator.id.name &&
...
function isIndexVarOnlyUsedToGetArrayElements(context, node) {
    ...
    const listText = sourceCode.getText(node.test.right.object)
    const iVar = context.getDeclaredVariables(node.init)[0]

    return iVar.references.every(reference => {
        const id = reference.identifier

        return !contains(node.body, id) || (
            id.parent.type === "MemberExpression" &&
            id.parent.property === id &&
            sourceCode.getText(id.parent.object) === listText
        )
    })
}
...
function getElementVariableDeclaration(sourceCode, node) {
    ...
    const arrayText = sourceCode.getText(node.test.right.object)
 ...
 */
function fixArrayForEach(callbackInfo, sourceCode, fixer) {
 ...
 * @returns {Fix|null} The created fix object.
 */
function fixForStatement(node, sourceCode, fixer) {
    ...
    function exitFunction() {
        if (funcInfo.isTarget) {
            ...
            context.report({
                ...
                message: MESSAGE,
                fix: fixArrayForEach.bind(null, funcInfo, sourceCode),
            })
        }
        ...
        "ForStatement:exit"(node) {
            if (isTraversingArray(node) &&
                ...
                context.report({
                    node,
                    message: MESSAGE,
                    fix: fixForStatement.bind(null, node, sourceCode),
                })
            }
        },
